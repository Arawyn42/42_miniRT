...
static double	cone_disc_intersection(t_ray ray, t_cone *co)
{
	...
	if (fabs(s) < PRECISION)
		return (INFINITY);
	center = add_vect(co->pos, multiply_vect_scalar(co->axis, co->height));
	ray_to_co = substract_vect(center, ray.origin);
	t = vect_dot(co->axis, ray_to_co) / s;
	r = vect_length(substract_vect(intersection_point(ray, t), center));
	if (r <= co->radius)
		return (t);
	return (INFINITY);
}
...
double	cone_intersection(t_ray ray, t_cone *co)
{
	...
	co_to_ray = substract_vect(ray.origin, co->pos);
